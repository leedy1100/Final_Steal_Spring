...
$(function() {
	...
	var isEdge = DetectRTC.browser.isEdge && browserVersion >= 15063; // 15버전
																		// 이상
		...
		navigator
				.getUserMedia(
						...
						function(stream) {
							...
							$videoWrap
									.append('<video id="local-video-medium" class="local-video" muted="muted" autoplay="true" title="180p"></video>');
		...
		navigator
				.getUserMedia(
						...
						function(stream) {
							...
							$videoWrap
									.append('<video id="local-video-small" class="local-video" muted="muted" autoplay="true" title="90p"></video>');
	...
	function createOffer(sessionType, peer, stream) {
		...
		peer.createOffer(function(SDP) {
			...
			if (location.search.substr(1).match('h264')) {
				SDP.sdp = SDP.sdp.replace('100 101 107', '107 100 101'); // for
																			// chrome
																			// < 57
				SDP.sdp = SDP.sdp.replace('96 98 100', '100 96 98'); // for
																		// chrome
																		// 57 <
			}
	...
	/**
	 * createAnswer offer에 대한 응답 SDP를 생성 한다.
	 * 
	 * @param {object}
	 *            msg offer가 보내온 signaling
	 */
	...
	/**
	 * createPeerConnection offer, answer 공통 함수로 peer를 생성하고 관련 이벤트를 바인딩 한다.
	 */
	function createPeerConnection(type) {
		...
		peer.pc.onaddstream = function(event) {
			...
			$videoWrap.append('<video id="' + id
					+ '" class="remote-video" autoplay="true"></video>');
		...
		peer.pc.oniceconnectionstatechange = function(event) {
			console.log('oniceconnectionstatechange', 'iceGatheringState: '
					+ peer.iceGatheringState, '/ iceConnectionState: '
	...
	/**
	 * getPeer 다수의 Peer중 해당하는 type과 매칭되는 peer 리턴한다.
	 */
	...
	function setClipboard() {
		...
		$uniqueToken.click(function() {
			...
			if (window.clipboardData) {
				...
				window.prompt('Copy to clipboard: Ctrl+C, Enter', link); // Copy
																			// to
																			// clipboard:
																			// Ctrl+C,
																			// Enter
	...
	function onFoundUser() {
		$roomList.html([ '<div class="room-info">',
				'<p>당신을 기다리고 있어요. 참여 하실래요?</p>',
				'<button id="join">Join</button>', '</div>', ].join('\n'));
